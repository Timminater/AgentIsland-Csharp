...
### 3. Provider 注册与新 Agent key 接入
下方示意代码展示能力实现和注册。全新 key 还必须补齐固定运行时映射：`DisplayProvider`、`DisplayProviders.Parse/All`、可见性与槽位选择，以及活动监控使用的 `TriggerTool` 和转换方法；按支持能力补齐 UI 名称、图标和导航映射。仅注册接口与目录不能启用未知 key，当前协调器会过滤它。除解析测试外，还需验证启用后的完整数据链路。
...
// 步骤 4：补齐上文的运行时/选择/UI 映射，并验证完整接入链路
```
