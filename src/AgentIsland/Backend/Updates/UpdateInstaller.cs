...
public static class UpdateInstaller
{
    ...
    public static async Task RunAsync(UpdateInfo info)
    {
        ...
        var dialog = UI.IslandDialog.ShowAppProgress(
            AgentIsland.UI.Localization.L10n.TrFormat("Updating to {0}…", info.Tag),
            AgentIsland.UI.Localization.L10n.TrFormat("Downloading update… {0}%", 0));
        try
        {
        ...
        catch (Exception error)
        {
            ...
            UI.IslandDialog.ShowUpdate(
                "Agent Island",
                AgentIsland.UI.Localization.L10n.Tr("Automatic update failed. You can download the new version manually from GitHub Releases."),
    ...
    private static async Task<string> DownloadAsync(UpdateInfo info, Action<int> onPercent)
    {
        ...
        var zipPath = Path.Combine(StagingDir, info.AssetName ?? "AgentIsland.Update.zip");
